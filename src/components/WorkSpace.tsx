...
import { Block, Chart as WorkSpace } from "../types/ucs";

const validateAndLoadUCS = (content: string): WorkSpace | string => {
...
function WorkSpace() {
  const [chart, setChart] = useState<WorkSpace | undefined>(undefined);
  ...
  const handleUploadUCS = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      // 譜面のDOMを低優先度で再レンダリング
      startTransition(() => {
        const result: WorkSpace | string = validateAndLoadUCS(content);
